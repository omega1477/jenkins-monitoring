...
then
        ...
        wget --no-check-certificate https://itaas-build.sby.ibm.com:9443/jnlpJars/jenkins-cli.jar
fi
UPDATE_LIST=$( java -jar jenkins-cli.jar -s https://itaas-build.sby.ibm.com:9443/ -noCertificateCheck  list-plugins --username "bpmbuild@sg.ibm.com" --password "t3stc0balt1" | grep -e ')$' | awk '{ print $1 }' );
if [ ! -z "${UPDATE_LIST}" ]; then
    echo Updating Jenkins Plugins: ${UPDATE_LIST};
    java -jar jenkins-cli.jar -s https://itaas-build.sby.ibm.com:9443/ -noCertificateCheck  install-plugin ${UPDATE_LIST} --username "bpmbuild@sg.ibm.com" --password "t3stc0balt1";
    java -jar jenkins-cli.jar -s https://itaas-build.sby.ibm.com:9443/ -noCertificateCheck  safe-restart --username "bpmbuild@sg.ibm.com" --password "t3stc0balt1";
fi
...
sed -i 's/git@github.ibm.com:gts-sla\/jenkins-backup-standby.git/git@github.ibm.com:gts-sla\/jenkins-backup-main.git/g' "$JENKINS_SCM_CONFIG"
sed -i 's/https:\/\/itaas-build-2.sby.ibm.com:9443\//https:\/\/itaas-build.sby.ibm.com:9443\//g' "$JENKINS_MAILER"
sed -i 's/https:\/\/itaas-build-2.sby.ibm.com:9443\//https:\/\/itaas-build.sby.ibm.com:9443\//g' "$JENKINS_LOCATION"
